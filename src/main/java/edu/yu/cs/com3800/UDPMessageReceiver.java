...
public class UDPMessageReceiver extends Thread implements LoggingServer {
    ...
    private boolean sendLeader(Message received) {
        ...
        return (receivedState == PeerServer.ServerState.LOOKING || receivedState == PeerServer.ServerState.OBSERVER) && (this.peerServer.getPeerState() == PeerServer.ServerState.FOLLOWING || this.peerServer.getPeerState() == PeerServer.ServerState.LEADING);
    }
    ...
    private boolean strayElectionMessage(Message received) {
        ...
        ElectionNotification receivedNotification = LeaderElection.getNotificationFromMessage(received);
        return receivedNotification.getState() != PeerServer.ServerState.LOOKING && this.peerServer.getCurrentLeader() != null;
    }
}